...
pub fn get_registry_data(input: &str, outpath: &str) -> Result<(), Error> {
    // SOFTWARE hive
    ...
    //sof_get_device_data(&software_hive, blabla, outpath.clone()).expect("Failed to get Device Data!");
    //sys_get_hid_data(&system_hive, blabla, outpath.clone()).expect("Failed to get HID Data!");
    ...
    //sys_get_usb_data(&system_hive, blabla, outpath.clone()).expect("Failed to get USB Data!");
